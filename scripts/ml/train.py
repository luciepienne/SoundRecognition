...
@click.option(
    "--dataset-path",
    default="/workspace/data/datasets/MNIST-audio/1",
...
@click.option(
    "--n-classes", default=10, help="Number of classes in the dataset to be trained"
)
@click.option(
    "--correlation-id", default=uuid.uuid4(), help="Correlation ID for the training job"
)
@click.option(
    "--model-yaml-config",
    default="/workspace/scripts/ml/configs/config-1.yaml",
...
def train(
    ...
    # default tag
    mlflow.set_tags({"environment": "testing"})

    # Load dataset
    train_set = RecordingsDataset(os.path.join(dataset_path, "train.tfrecords"))
    validation_set = RecordingsDataset(
        os.path.join(dataset_path, "validation.tfrecords")
    )
