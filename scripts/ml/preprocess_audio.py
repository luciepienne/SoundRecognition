...
@click.option(
    "--output-dir", help="Path to directory where you want to output the tf records"
)
...
def process(
    ...
    processor = AudioToTFRecordProcessor(audio_dir=audio_dir, output_dir=output_dir)
    ...
    train_split, validation_split = processor.get_train_validation_split(
        files, train_ratio=train_split
    )
    splits = [
        {"name": "train", "files": train_split},
        {"name": "validation", "files": validation_split},
    ]
